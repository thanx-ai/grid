...
| ------------------------------------------------------- | ------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Every `<style>…</style>` block in `<head>`              | `index.css`                                             | Concatenate in source order. Strip any `@import url('https://fonts…')` lines (esbuild warns on remote `@import`, **and** an external font `<link>` is CSP-blocked in production) — self-host the font as inlined base64 `@font-face` instead (see Step 8 → "Remote font imports").                                                                                              |
...
- **Keep (or add) the `import "./index.css";` line.** This is the canonical mount point for the stylesheet — `App.tsx`, not `index.tsx` (importing in both double-loads it). esbuild only emits `dist/bundle.css` when CSS is in the entry import graph: if `App.tsx` does not import the stylesheet, the app renders **fully unstyled** and **`wmill app lint` still passes** (a missing CSS import is not a lint error). Source apps usually have this import already; a hand-translated `html` port often does not — add it. Step 9 verifies `dist/bundle.css` is non-empty.
- **Preserve icon/component size defaults when porting** (see the `public/` SVG note in this step) — a ported icon that spreads `{...props}` onto `<svg>` without a default `size`/`width`/`height` renders at full intrinsic size for callers that pass only `className`.

**`index.css`:** copy as-is; confirm `App.tsx` imports it (see above).
...
2. For each file, decide:
   - Small SVG (< 5KB) → convert to an inline React component. **Preserve the size default.** If the source icon hardcoded `width`/`height` (or exposed a `size` prop with a default), keep it. Rewriting the icon to spread `{...props}` onto `<svg>` without a default `size`/`width`/`height` makes callers that pass only `className` render the SVG at its full intrinsic size (often hundreds of px) — lint-clean, visually broken.
   - Small PNG/JPG (< 5KB) → inline as a base64 `data:` URL.
   - Anything larger or many files → ask the user via `AskUserQuestion` whether to skip (don't copy), externalize through a `load_assets` runnable + `useAsset()` hook (see below), inline into a Windmill resource at `<SCOPE>/<name>_assets`, or upload to an external CDN. Surface this as a blocker the user must resolve before Step 9.

**Asset-key consistency for the `load_assets` + `useAsset()` pattern.** When assets are externalized through a `load_assets` runnable that returns a keyed map and a frontend `useAsset(key)` hook that requests by key, **the key the runnable emits and the key the frontend requests must come from the same derivation** — otherwise the image silently renders blank, and a wrong asset key is **not** a lint or type error. Pick one canonical key function and apply it identically on both sides: take the public-relative path, strip the leading `/`, drop the extension, replace every `[^a-z0-9]+` run with `_`, lowercase. So `/demo-assets/brand/cover-image.jpg` → `demo_assets_brand_cover_image`. Define it once (e.g. a shared `assetKey(path)` helper used by both the runnable and the frontend) rather than hand-writing keys on either side.
...
- **Remote font imports** (`@import url('https://fonts.googleapis.com/...')`, or any other remote `@import url(...)` line): remove them from `index.css`. Two reasons, not one: (a) esbuild emits a warning on a remote CSS `@import url(...)` because it cannot bundle a remote URL — CI-fatal; (b) **even an external runtime `<link>` to `fonts.googleapis.com` / `fonts.gstatic.com` is blocked by the deployed app's Content-Security-Policy.** The `<link>` loads fine under `wmill app dev` (no CSP) but in production the request is refused and the font **silently falls back to a system font — a production-only regression that lints clean and looks correct in local dev.** (Next.js avoids this because `next/font` self-hosts; this is the same CSP class as the no-external-CDN constraint.) **Do NOT inject an external `<link>`.** Self-host instead:
  1. Download the `.woff2` file(s) for each weight/style the design actually uses (from the Google Fonts CSS the source referenced, or a tool like `google-webfonts-helper`).
  2. Base64-encode each and inline them as `@font-face` rules in `index.css` (the bundled stylesheet `App.tsx` imports), so the font ships inside `dist/bundle.css` with no external request:
     ```css
     @font-face {
       font-family: "Inter";
       font-weight: 400;
     ...
     ```
  3. One `@font-face` per weight/style; keep only the weights the design uses (each inlined weight adds ~15–30 KB of base64 to the bundle). After building, confirm the font actually applied — see Step 9's visual check; a missing or misnamed `@font-face` is invisible to lint.
...
2. **Confirm the stylesheet and assets actually shipped — lint will not.** After lint runs its `npm install` + build, check that `dist/bundle.css` exists and is non-empty, **and** that `App.tsx` contains `import "./index.css";`. A missing import yields a fully **unstyled** app that **lints clean** (a missing CSS import is not a lint error). Same blind spot for images: a self-hosted font whose `@font-face` name does not match the `font-family` in the CSS, or a `useAsset(...)` key that does not match the `load_assets` key (Step 8), renders the fallback font / a blank image with no lint or type error. These only surface visually — see item 4.
3. For **any new inline runnable**, OR **any source mode `html` import**, run `wmill app dev <SCOPE>/<name>.raw_app` and load `http://localhost:5173` in a browser before declaring success:
   ...
   - If `wmill workspace list` is empty, follow `claude/rules/local-windmill-dev.md` to add the `local` workspace before this step.
4. **Lint-green ≠ visually correct.** Every failure mode this skill warns about — unstyled app (dropped CSS import), system-font fallback (CSP-blocked external font), blank images (mismatched asset key), oversized icons (lost size default), re-initializing charts — passes `wmill app lint` and renders without a thrown error. The only reliable catch is **looking at the rendered output against the source.** When you have the original (a running source app, the source `.html`, or deployed reference), do a per-route/per-view **screenshot + pixel diff** sweep (e.g. drive both with Playwright and compare with `pixelmatch`); otherwise eyeball each view in the `wmill app dev` browser tab against the source. Treat a visual mismatch as a blocker just like a lint warning.
5. **Manually verify every literal `wmill.getVariable("f/...")` / `wmill.getResource("f/...")` you wrote into a `backend/*.yaml` inline script exists in the prod workspace.** `bash scripts/check-variable-references.sh` only scans `*.ts` / `*.py` / `*.go` (see the `--include` flags in the script) — it does **not** read inline YAML content, so the CI check will silently pass for unresolved variable refs buried inside `inlineScript.content`. Until that gap is closed, the check is on you. Run e.g. `wmill variable list --workspace thanx | grep <SCOPE>/<NAME>` for each literal, or hit `GET /api/w/thanx/variables/exists/<SCOPE>/<NAME>` with your prod token. If any path is missing, create the variable in the Windmill UI before deploying. See `claude/rules/scaffold-getvariable-placeholders.md` for the failure mode this prevents.
...
Manual checks:<list of wmill.getVariable / getResource paths the user must confirm in the workspace, per Step 9 item 5>
...
- **Remote fonts: strip the `@import`, self-host — do NOT inject an external `<link>`.** A remote CSS `@import url('https://…')` warns under esbuild (CI-fatal), and an external `<link>` to `fonts.googleapis.com`/`fonts.gstatic.com` is **blocked by the deployed app's CSP** — it works in `wmill app dev` but silently falls back to a system font in production. Inline the woff2 as base64 `@font-face` in `index.css` instead. See Step 8 → "Remote font imports".
- **Lint-green is not visually correct.** Dropped CSS import (unstyled app), CSP-blocked font (system-font fallback), mismatched `useAsset`/`load_assets` key (blank image), lost icon size default (giant SVG) — all pass `wmill app lint` and render without throwing. Verify visually against the source (screenshot + pixel diff, or eyeball each view in `wmill app dev`). See Step 9 item 4.
