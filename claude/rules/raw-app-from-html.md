...
The deployed loader (`f/<team>/load_<name>.ts`) holds the data and **must ship with a colocated `f/<team>/load_<name>_test.ts`** (per `CLAUDE.md` → "Deploy tests"). The test's first line is `// test: script/f/<team>/load_<name>` so `bash scripts/run-deploy-tests.sh` discovers and runs it on every deploy; without it, a regression in the loader's return shape only surfaces when the raw_app crashes in production. At minimum the test should assert non-empty primary arrays, expected top-level keys, and that numeric fields are numbers. The test must `import { main } from "./<sibling>.ts"` — see `claude/rules/deploy-test-no-nested-job.md` and `f/shared/load_customer_cube_overview_test.ts` for why `wmill.runScript` is unsafe in a deploy test on Grid.
